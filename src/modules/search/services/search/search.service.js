  ...
  try {
    ...
    if (filters.project_id) {
      ...
      if (scope?.subjectCategoryIds?.length > 0) {
        const topicRes = await prisma.$queryRawUnsafe(
          'SELECT topic_id FROM "Topic" WHERE subject_category_id = ANY($1::bigint[])',
          scope.subjectCategoryIds
        );
        scope.inScopeTopicIds = topicRes.map((r) => Number(r.topic_id));
      } else {
    ...
    console.time(`==> Phase 1 & 2: PostgreSQL [${traceId}]`);
    const searchPromises = selectedTypes.map(async (entityType) => {
      const buildQuery = BRANCH_BUILDERS[entityType];
      if (!buildQuery) return;
      
      const typeParamsCount = [...params];
      const countSql = buildQuery(typeParamsCount, context, 'count');
      
      const typeParamsList = [...params];
      const listSql = buildQuery(typeParamsList, context, 'light');
      
      try {
        const [countRes, listRes] = await Promise.all([
           prisma.$queryRawUnsafe(countSql, ...typeParamsCount),
           prisma.$queryRawUnsafe(listSql, ...typeParamsList)
        ]);
        
        const hitsCount = Number(countRes[0]?.count || 0);
        rawCounts[entityType] = hitsCount;
        // NOTE: total is sum of raw counts, but here it's mutated in async.
        // It's safer to sum after Promise.all
        ...
        logger.error(`Error executing Postgres search for ${entityType}:`, err);
      }
    });
    ...
    // Sum total
    for (const type of selectedTypes) {
       total += (rawCounts[type] || 0);
    }
    
    console.timeEnd(`==> Phase 1 & 2: PostgreSQL [${traceId}]`);
    ...
    const hydrationPromises = Object.keys(groupedIds).map(async (entityType) => {
      ...
      try {
        const hydrateRes = await prisma.$queryRawUnsafe(sql, ...typeParams);
        return Array.isArray(hydrateRes) ? hydrateRes : [];
      } catch (err) {
